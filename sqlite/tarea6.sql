...
SELECT producto_id, SUM(cantidad) FROM pedidos GROUP BY producto_id ORDER BY SUM(cantidad) DESC;
...
SELECT * FROM pedidos WHERE id_cliente = 2 AND strftime('%Y-%m', fecha) = '2024-02';
...
SELECT DISTINCT id_cliente FROM pedidos WHERE strftime('%Y-%m', fecha) = '2024-02';

-- Obtener la fecha del último pedido realizado por el cliente con ID 3.
SELECT MAX(fecha) FROM pedidos WHERE id_cliente = 3;
...
SELECT * FROM clientes WHERE (nombre GLOB '*[aeiouAEIOU]*') AND (nombre GLOB '*[aeiouAEIOU]*') AND (nombre GLOB '*[aeiouAEIOU]*') AND (nombre GLOB '*[aeiouAEIOU]*');
...
SELECT * FROM productos WHERE nombre GLOB '*[bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ]';
